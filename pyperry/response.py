...
class Response(object):
    ...
    A write adapter returns a Response object for all calls to its write or
    delete methods. The write adapter is responsible for setting the success,
    raw, raw_format, status, and meta attributes on the Response object, but
    ...
    middleware may expire some cache entries for that model. The parsed,
    model_attributes, and errors methods are provided to return the response
    data in formats that are easy for middlewares to work with.

    Response attributes

    success - set to True if the write or delete succeeded
    raw - raw response data from the adapter
    raw_format - format of the raw response, such as 'json' or 'xml'
    status - adapter-specific status code (optional)
    meta - adapter-specific information about the response (optional)
